...
pub fn mask(comptime dim: comptime_int, input: @Vector(dim, i32)) @Vector(dim, i32) {
    return input;
}

/// Computes the cross product of two 3D vectors.
pub fn cross(comptime T: type, lhs: @Vector(3, T), rhs: @Vector(3, T)) @Vector(3, T) {
...
pub fn lengthSquared(comptime dim: comptime_int, comptime T: type, v: @Vector(dim, T)) T {
    return @reduce(.Add, v * v);
}

/// Computes the length of the provided vector.
pub fn length(comptime dim: comptime_int, comptime T: type, v: @Vector(dim, T)) T {
    return @sqrt(lengthSquared(dim, T, v));
}

/// Computes the dot product of two vectors.
pub fn dot(comptime dim: comptime_int, comptime T: type, lhs: @Vector(dim, T), rhs: @Vector(dim, T)) T {
    return @reduce(.Add, lhs * rhs);
}

/// Creates a new SIMD vector with the same elements as `v`, but truncate to `new_dim`.
pub fn truncate(comptime T: type, comptime dim: comptime_int, new_dim: comptime_int, v: @Vector(dim, T)) @Vector(new_dim, T) {
...
pub fn normalize(comptime dim: comptime_int, comptime T: type, v: @Vector(dim, T)) @Vector(dim, T) {
...
/// `@as()` cast.
pub fn splat(comptime dim: comptime_int, value: anytype) @Vector(dim, @TypeOf(value)) {
    return @splat(value);
}
